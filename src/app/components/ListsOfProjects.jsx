...
const fetchPosts = async () => {
    return fetch('https://myblog-ar9m5tg6pv.live-website.com/wp-json/wp/v2/posts?categories=2', {
        next: {
            revalidate: 60
        }
    }).then(res => res.json())
...
export default async function ListsOfProjects() {
    const posts = await fetchPosts()
    return (<div className="grid grid-cols-1 gap-4 font-thin sm:grid-cols-2 md:grid-cols-3">{
                    ...
                    <article className="border border-gray-300 dark:border-neutral-700 hover:bg-gray-100 hover:dark:bg-neutral-800/30" key={post.id}>
                        <div className="relative w-full h-44">
                            <Image
                                ...
                                fill
                                sizes="100vw"
                                className='dark:drop-shadow-[0_0_0.3rem_#ffffff70] object-cover aspect-square'
                                priority
                            />
                        </div>
